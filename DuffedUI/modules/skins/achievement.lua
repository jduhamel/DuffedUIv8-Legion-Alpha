...
local function LoadSkin()
	...
	for _, frame in pairs(frames) do _G[frame]:StripTextures(true) end
	...
	for _, frame in pairs(noname_frames) do
		for i=1, _G[frame]:GetNumChildren() do
			...
			if child and not child:GetName() then child:SetBackdrop(nil) end
		end
	end
	...
	local function SkinStatusBar(bar)
		...
		if _G[bar:GetName().."Title"] then _G[bar:GetName().."Title"]:SetPoint("LEFT", 4, 0) end
		if _G[bar:GetName().."Label"] then _G[bar:GetName().."Label"]:SetPoint("LEFT", 4, 0) end
		if _G[bar:GetName().."Text"] then _G[bar:GetName().."Text"]:SetPoint("RIGHT", -4, 0) end
	end
	...
	for _, compare in pairs(compares) do
		for i = 1, 9 do
			...
			_G[frame].bg1:SetTexture(C["media"].glossTex)
			...
			if compare == "Friend" then _G[frame.."Shield"]:Point("TOPRIGHT", _G["AchievementFrameComparisonContainerButton"..i.."Friend"], "TOPRIGHT", -20, -3) end
